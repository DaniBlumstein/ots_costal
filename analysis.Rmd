...
write.csv(combined_1[, 1:17], file = "Ots_coastal_2024_meta.csv")

genos_all <- combined_1[c(-2:-17)]
write.csv(colnames(genos_all), file = "Ots_coastal_2024_loci.csv")

colnames(genos_all)
meta<-read.csv("Ots_coastal_2024_meta.csv", header = TRUE)

##read marker information for Ots GT-seq panel
ots_panel_info<-read_excel("Ots GT-seq panel info.xlsx",sheet = 2)
```
...
run_timing<-genos_all[, c(1, which(colnames(genos_all) %in% rt_markers$Assay))]

rt_matrix <- as.matrix(run_timing[,c(2:(ncol(run_timing)))]) 
row.names(rt_matrix) <- run_timing$sample
...
rt_PCA<-ggplot(snp_pcs)+
  geom_point(aes(Axis1, Axis2, color = Population, shape = run), position = jitter, alpha = 0.5) +
  theme_classic()+
  ggtitle("RT markers")
...
ggplot(snp_pcs)+
  geom_point(aes(Axis1, Axis2, color = sex), position = jitter, alpha = 0.5) +
...
neutral_matrix<-as.matrix(genos_all[, colnames(genos_all) %in% neutral_marker_list$Assay])
...
```{r}
non_run_timing<-genos_all[, c(which(!colnames(genos_all) %in% rt_markers$Assay))]
non_run_timing_matrix<-as.matrix(non_run_timing[, c(2:(ncol(non_run_timing)))])
...
non_run_timing_PCA<-ggplot(snp_pcs)+
  geom_point(aes(Axis1, Axis2, color = stream), alpha = 0.5) +
  theme_classic()+
  ggtitle("non_run_timing markers")

##PCA sex 
non_run_timing_sex_PCA<-ggplot(snp_pcs)+
  geom_point(aes(Axis1, Axis2, color = sex), position = jitter, alpha = 0.5) +
  theme_classic()+
  ggtitle("non_run_timing_sex markers")
...
non_run_timing_genind$pop<-as.factor(nm_df$`meta$run`)
non_run_timing_loci<-genind2loci(non_run_timing_genind) 
